...
function createData(
  ...
  return { transadate,pn,reference,narrative,transactionanalysis,debit,credit,user,source,urn };
}

const rows = [
  createData('02/10/2021','','','','',0,0,'','',''),
  createData('02/10/2021','','','','',0,0,'','',''),
  createData('02/10/2021','','','','',0,0,'','',''),
  createData('02/10/2021','','','','',0,0,'','',''),
  createData('02/10/2021','','','','',0,0,'','',''),
...
const headCells: HeadCell[] = [
  { id: 'transadate', numeric: false, disablePadding: true, label: 'Trans Date' },
  { id: 'pn', numeric: true, disablePadding: false, label: 'PN' },
...
export default function EnhancedTable() {
  ...
  const [orderBy, setOrderBy] = React.useState<keyof Data>('transadate');
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelecteds = rows.map((n) => n.transadate);
          ...
          >
            ...
            <TableBody>
              {stableSort(rows, getComparator(order, orderBy))
                ...
                .map((row, index) => {
                  const isItemSelected = isSelected(row.transadate);
                  const labelId = `enhanced-table-checkbox-${index}`;

                  return (
                    <TableRow
                      hover
                      onClick={(event) => handleClick(event, row.transadate)}
                      role="checkbox"
