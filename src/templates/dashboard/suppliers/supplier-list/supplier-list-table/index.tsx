...
import SupplierTxnEnquiryModal from "../supplier-txn-enquiry-modal";
...
export default function SupplierListTable() {
  ...
  const [customerTxnModal, setSupplierTxnModal] = React.useState(false);

  const handleSupplierTxnModal = (event: boolean) => {
    setSupplierTxnModal(event);
  };
  ...
  return (
    <div className={classes.root}>
            ...
            <TableBody>
              {stableSort(rows, getComparator(order, orderBy))
                ...
                .map((row, index) => {
                  ...
                  return (
                    <TableRow hover tabIndex={-1} key={row.code} onClick={()=>handleSupplierTxnModal(true)} style={{cursor:'pointer'}}>
      ...
      <SupplierTxnEnquiryModal
        open={customerTxnModal}
        handleModal={handleSupplierTxnModal}
      />
    </div>
  );
}
