...
const CurrencySelection: React.FC<{
  ...
  currencySelectionhandleChange: (
    event: React.ChangeEvent<{
      value: unknown;
    }>
  ) => void;
  currencySelectionOptions: { name: string; value: string }[];
}> = ({
  currencySelected,
  currencySelectionhandleChange,
  ...
  return (
    <Grid container spacing={3}>
      <Grid item xs={12} md={3} sm={6}>
        <FormControl className={classes.formControl} margin={margin}>
          ...
          <Select
            ...
            onChange={currencySelectionhandleChange}
          >
            {currencySelectionOptions.map(
      ...
      {currencySelected === "is" && <IsSelection />}
      {currencySelected === "in" && <InSelection />}
    </Grid>
  );
};
